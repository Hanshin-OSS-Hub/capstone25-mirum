...
/**
 * @param {{ item: import('@/types/file.js').FileListItem, onDownload?: (item: import('@/types/file.js').FileListItem) => void }} props
 */
      ...
      <div className="mt-5 border-t border-gray-100 pt-4">
        <div className="flex items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-2">
            <UserProfileImg name={item.uploadedBy} size="md" />
            <span className="truncate text-sm text-gray-600">{item.uploadedBy}</span>
          </div>

          <div className="flex items-center gap-2">
            <span className="shrink-0 text-sm text-gray-500">{item.displayDate}</span>

            {/* 향후 다운로드 버튼을 다시 사용하고 싶다면, 아래 코드를 언주석 처리하고 onDownload를 연결하세요. */}
            {/*<button*/}
            {/*  type="button"*/}
