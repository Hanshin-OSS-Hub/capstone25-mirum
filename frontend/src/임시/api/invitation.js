...
export const invitationHandlers = [
  ...
  http.get('*/api/invitations/sent/:projectId', ({ params, request }) => {
    ...
    // 프로젝트별 초대 목록 필터링
    const invitationList = database.invitations.filter(
      (i) => i.projectId === Number(projectId) && i.status === 'INVITED',
    );

    console.log('MSW: 프로젝트별 보낸 초대 목록 조회');
    return successResponse(invitationList, 200);
  }),
  ...
  http.get('*/api/invitations/received', ({ request }) => {
    ...
    const invitationList = database.invitations.filter(
      (i) => i.invitedName === userData.username && i.status === 'INVITED',
    );
  ...
  http.post('*/api/invitations/:inviteId/accept', async ({ params, request }) => {
    ...
    console.log(
      `MSW: 프로젝트 초대 수락 성공 (프로젝트 ID: ${invitation.projectId}) 유저 ID: ${user.id})`,
    );
  ...
  http.put('*/api/invitations/:inviteId/decline', async ({ params, request }) => {
    ...
    console.log(
      `MSW: 프로젝트 초대 거절 성공 (프로젝트 ID: ${invitation.projectId}) 유저 ID: ${user.id})`,
    );
    return successResponse(null, 200);
  }),
];
